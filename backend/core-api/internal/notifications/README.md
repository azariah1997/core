...
- **Send is self-or-platform.admin.** A user can always notify themselves; only a platform.admin can trigger a notification to someone else. This platform has no caller identity distinct from "a user" yet (no service-account concept), so self-or-admin is the safe default rather than letting any authenticated caller spam any other user - the same pattern Phase 6 established for cross-user profile access.
